...
z = ReferenceToStretchedDiscretization(; extent=5000,
    constant_spacing=10.0,
    constant_spacing_extent=100,
    stretching=PowerLawStretching(1.15))

Nx = 1 * Int(λ₂ - λ₁) # 1/15 th of a degree resolution
Ny = 1 * Int(φ₂ - φ₁) # 1/15 th of a degree resolution
Nz = length(z)
...
FT = DatasetRestoring(temperature_metadata; rate=1 / 30days)
FS = DatasetRestoring(salinity_metadata; rate=1 / 30days)
...
set!(ocean.model, MetadataSet(:temperature,
    dataset=GLORYSMonthly(), date=start_date))

fig = Figure()
ax = Axis(fig[1, 1])
heatmap!(ax, interior(ocean.model.tracers.T, :, :, Nz), colorrange=(10, 20), colormap=:thermal)
ax = Axis(fig[1, 2])
heatmap!(ax, interior(ocean.model.tracers.S, :, :, Nz), colorrange=(35, 40), colormap=:haline)
